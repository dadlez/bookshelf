import { getBooksResponseSchema, type GetBooksResponse, AppError, type FilterParams, type OrderParams, type SearchParams } from "@bookshelf/shared";

const VALIDATE = import.meta.env.VITE_VALIDATE_RESPONSES === "true";

export async function fetchBooks(cursor?: string, filterParams?: FilterParams, orderParams?: OrderParams, searchParams?: SearchParams): Promise<GetBooksResponse> {
  ...
  if (orderParams?.sortBy) url.searchParams.set("sortBy", orderParams.sortBy);
  if (orderParams?.sortOrder) url.searchParams.set("sortOrder", orderParams.sortOrder);
